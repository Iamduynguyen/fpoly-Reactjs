  ...
  return (
    <div>
      <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 className="h2">Fashion Watch</h1>
        <form className="d-none d-sm-block  mr-auto ml-md-5 my-25 my-md-0 mw-150 ">
          <input id='seach' onKeyUp={props.onSeach} type="text"
        ...
        <div className="btn-toolbar mb-2 mb-md-0 pd-5">
          <Link to="/admin/Fashion/add" className="btn btn-primary me-5 " >Them san pham</Link>
        </div>
      </div>
            ...
            {props.products.map(function (product, index) {
              return (
                <tr key={index}>
                  ...
                  <td><Link>{product.name}</Link></td>
                  <td>
                    <img src={product.img} height='80' /></td>
                  <td>{product.price}
                  </td>
                  <td>{product.quantity > 0 ? <span className="chuxanh">Còn hàng</span> : <span className="pagenumber">Hết hàng</span>}</td>
                  <td>{product.quantity}</td>
                  <td>
                    <Link className="btn btn-primary" to={`Fashion/${product.id}`}>Sua</Link>
