...
const AdminDiamondwatch = () => {
  ...
  useEffect(() => {
    const getProducts = async () => {
      const respose = await fetch(`http://localhost:3001/diamond-Watch?_limit=6`);
  ...
  const getproductpage = async (page) => {
    const respose = await fetch(`http://localhost:3001/diamond-Watch?_page=${page}&_limit=6`);
    const data = await respose.json();
    setProducts(data);
    setPage(page);
  }
...
export default AdminDiamondwatch